...
//               synchronized 同步锁        防止 这段代码在执行的时候，     if(!isRegisterSucceed){
//                        xmppManager.dropTask(1);
//                        xmppManager.runTask();
//                        xmppManager.startReconnectionThread();
//                    } 也在执行
// xmppManager.runTask();并行
                ...
                try {
                    Thread.sleep(10 *1000); //在10s中内，服务器完全有时间处理我们的请求，并作出响应
        ...
        public void run() {
            ...
            if (!xmppManager.isAuthenticated()) {
                ...
                try {
                    ...
                    connection.addPacketListener(packetListener, packetFilter);
                    connection.startHeartBeat(); //建立连接的时候启动心跳包
                    synchronized (xmppManager)
                    {
                    ...
                    xmppManager.runTask(); //保证runTask能一定执行到，避免return的情况。
                }
